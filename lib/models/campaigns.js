...
module.exports.delivered = function(id, callback) {
    id = Number(id) || 0;
    ...
    let query = 'SELECT COUNT(*) as delivered FROM `campaign__' + id + '`';
    db.getConnection((err, connection) => {
        if (err) {
            return callback(err);
        }
        connection.query(query, (err, count) => {
            ...
            return callback(null, count[0].delivered)
        })
    })
}


module.exports.bounced = function(id, callback) {
    id = Number(id) || 0;
    ...
    let query = 'SELECT COUNT(*) as bounced FROM `campaign__' + id + '` WHERE `campaign__' + id + '`.`status` = ? ';
    db.getConnection((err, connection) => {
        if (err) {
            return callback(err);
        }
        connection.query(query, [3], (err, count) => {
            connection.release();
            if (err) {
                return callback(err);
            }
            return callback(null, count[0].bounced)
        })
    })
}